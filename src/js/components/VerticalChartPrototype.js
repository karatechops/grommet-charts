...
export default class VerticalChartPrototype extends Component {
  ...
  _bounds (series, xAxisArg, width, height) {
    ...
    series.forEach((item) => {
      item.values.forEach((value, xIndex) => {
        let x, y;
        if (Array.isArray(value) && this.props.orientation === "horizontal") {
          x = value[0];
          y = value[1];
        ...
        if (Array.isArray(value) && this.props.orientation === "vertical") {
          x = value[1];
          y = value[0];
    ...
    if (null === minX && this.props.orientation === 'horizontal') {
      minX = 0;
      maxX = 1;
      minY = 0;
      maxY = 100;
    } else if (null === minY && this.props.orientation === 'vertical') {
      minX = 0;
      maxX = 100;
      minY = 0;
      maxY = 1;
  ...
  _coordinates (value, orientation = 'horizontal') {
    /*let x, y;
    ...
    let x, y;
    if (Array.isArray(value)) {
      x = (orientation === 'horizontal') ? value[0] : value[1];
      y = (orientation === 'horizontal') ? value[1] : this.state.bounds.maxY - value[0];
    } else {
      x = (orientation === 'horizontal') ? value.x : value.y;
      y = (orientation === 'horizontal') ? value.y : this.state.bounds.maxY - value.x;
    }
  ...
  _renderLinesOrAreas () {
    ...
    let values = this.props.series.map((item, seriesIndex) => {
      ...
      let coordinates = item.values.map((value) => {
        return this._coordinates(value, this.props.orientation);
      });
  ...
  render () {
    ...
    let role = 'img';
    if (this.props.legend) {
      frontBands = this._renderHotspots('front');
      activeDescendant = (
...
VerticalChartPrototype.propTypes = {
...
VerticalChartPrototype.contextTypes = {
  intl: PropTypes.object
};

VerticalChartPrototype.defaultProps = {
